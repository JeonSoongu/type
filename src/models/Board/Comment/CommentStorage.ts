...
class CommentStorage {
  ...
  static findAllByBoardNum(boardNum : number) : Promise<RowDataPacket[]> {
    return new Promise((resolve, reject) => {
      ...
      db.query(query, [boardNum], (err, comment : RowDataPacket[]) => {
        if (err) reject(err);
        resolve(comment);
      });
    });
  ...
  static deleteCommentByNum(num : number) : Promise<number> {
    return new Promise((resolve, rejcet) => {
      const query = "DELETE FROM comments WHERE no = ? AND reply_flag = 0 AND depth = 0";

      db.query(query, [num], (err, row : ResultSetHeader) => {
        if (err) rejcet(err);
