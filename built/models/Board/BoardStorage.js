...
var BoardStroage = /** @class */ (function () {
    ...
    BoardStroage.findAllByCategoryNum = function (categoryNum, lastNum) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [categoryNum, lastNum], function (err, boards) {
                if (err)
                    reject(err);
                else
                    resolve(boards);
    ...
    BoardStroage.findOneByNum = function (num) {
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [num], function (err, board) {
                if (err)
                    reject(err);
                else
                    resolve(board);
    ...
    BoardStroage.findOneWatchListFlag = function (studentId, num) {
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [studentId, num], function (err, watchList) {
                if (err)
                    reject(err);
                else
    ...
    BoardStroage.updateByNum = function (board, num) {
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [board.title, board.content, board.price, num], function (err, boards) {
                if (err)
                    reject(err);
                else
                    resolve({ success: true, num: boards.insertId });
    ...
    BoardStroage.updateOnlyStatusByNum = function (board, num) {
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [board.status, num], function (err) {
                if (err)
                    reject(err);
                else
                    resolve(true);
    ...
    BoardStroage.findAllByIncludedTitleAndCategory = function (title, categoryNum) {
        return new Promise(function (resolve, reject) {
            ...
            db_1.default.query(query, [title, categoryNum], function (err, boards) {
                if (err)
                    reject(err);
                else
                    resolve(boards);
